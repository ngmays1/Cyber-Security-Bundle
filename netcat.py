...
def client_handler(client_socket):
    ...
    if len(upload_destination):
        ...
        try:
            ...
            #acknowledgethat we wrote the file out
    ...
    if command:
        while True:
            #show a simple promopt
            client_socket.send("<BHP:#> ")
            #recieve until see linfeed (enter key)
            cmd_buffer = ''
            while '\n' not in cmd_buffer:
...
def main():
    ...
    #are we going to listen or just send data fom stdin?
    ...
    #listen and potentiall upload things, execute commands
    #drop a shell back depending on cmd line options above
    if listen:
        server_loop()
main()
